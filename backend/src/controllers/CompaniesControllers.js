...
module.exports = {
    ...
    async search_max_companies (req,res){
        let companies_max = await db.query("SELECT COUNT(`adjudicatario`) AS count, group_concat(DISTINCT `adjudicatario`) FROM `informacao` WHERE `adjudicante` LIKE '%Hospital%' AND `dataPub` BETWEEN '2008-01-01' AND '2011-12-31' GROUP BY `adjudicatario` ORDER BY count DESC")
